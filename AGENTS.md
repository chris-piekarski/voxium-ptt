...
- **Development** optional extras: `black`, `pylint`, `ruff`, `pytest`, `pytest-cov`, `mypy`, `httpx`, `py-spy`, type stubs (see `[project.optional-dependencies] dev`).
...
| Area | Role |
|------|------|
| `src/voxium/` | Application code: `app.py` (client UI / hotkeys / **PTT & VOX** input modes), `vox_chunker.py` (utterance end-pointing for **open-mic VOX**), `whisper_server.py` (FastAPI + inference), `cli/`, `config`, `model_registry`, **shared local GGUF lane** (`polish_*`, `llama_cpp_*`, `ux_chatter*` uses the selected polish model), **session UI** (`console_status`, `recording_ui`, `startup_banner`, `standby_fft`, `standby_telemetry`), **slash** (`slash_commands`, `slash_complete`), `session_history` (in-RAM PTT/VOX transcript list; `/history`), `persistent_stats` (local `/stats` counters in `~/.config/voxium/stats.json`), `disk_usage_report` (`make disk-usage` / `/disk`), and **small pure modules** (`loopback`, `hotkey_rules`, `metrics_*`, `speech_guards`, etc.) for testable logic |
...
**Repository data on disk** (what `make disk-usage` and `/disk` summarize): **`models/`** (transcribe cache plus the shared `models/polish/` GGUF lane used by both re-encode and UX chatter; older `models/ux/` caches may exist but are not the active model lane), **`logs/`**, and **`tools/llama.cpp/`** (local `llama-server` runtime/prebuilds) under the project root (or `VOXIUM_REPO_ROOT`). `make clean` removes **`tools/llama.cpp/`** in addition to caches. **Operator config and long-running stats** live outside the repo under `~/.config/voxium/config.yaml` and `~/.config/voxium/stats.json`; defensive `.gitignore` entries cover accidental repo-local `.config/voxium/` files. **Transcript text** for `/history` and replay is **in-process RAM only** — the product does not use a `history/` directory under the repo for that data.
...
- **UX chatter uses the polish model lane:** default is on unless `--no-ux-chatter`, `ux_chatter.enabled: false`, or `VOXIUM_UX_CHATTER=0` disables it. It uses the **same** `llama-server` on `--llama-cpp-url` and the **same** `--polish-model` selected by `/models polish use <id>`; there is no separate active UX model or port. It only affects console copy (startup, standby/copy wit, log footer, edge line, sign-off), not STT/paste output.
...
- **Hotkeys from slash commands:** `/hotkeys ptt <f1..f12>` persists `hotkeys.record`; `/hotkeys replay <f1..f12>` persists `hotkeys.recovery`. Keep operator-facing command names (`ptt`, `replay`) distinct from canonical config keys (`record`, `recovery`), and preserve unrelated YAML sections when writing `~/.config/voxium/config.yaml`.
- **Persistent `/stats`:** Count only audio that reaches server `/transcribe` (PTT, VOX utterances, and F6 re-transmit). Locally filtered too-short/no-speech captures do not count. Store long-running totals in `~/.config/voxium/stats.json`, not `config.yaml`; `/stats` may also show server-process `/stats`, which resets with server restart.
